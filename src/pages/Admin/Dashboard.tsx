...
import { FileText, FolderOpen, TrendingUp, Eye, Share2, MessageSquare } from "lucide-react"
...
const Dashboard = () => {
  ...
  const calculateStats = (): DashboardStats => {
    ...
    const recentArticles = articles.filter((a) => new Date(a.createdAt) > sevenDaysAgo).length
    ...
    if (Object.keys(categoryCount).length > 0) {
      const popularSlug = Object.keys(categoryCount).reduce((a, b) => (categoryCount[a] > categoryCount[b] ? a : b))
          ...
          {!isLoading && (
            <>
                ...
                {statCards.map((stat, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className={`text-3xl font-bold ${stat.iconColor} mb-1`}>{stat.value}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Most Popular Category</h3>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.popularCategory}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Category with most articles</p>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600 dark:text-gray-400">Avg. Views per Article</span>
                      <span className="text-lg font-bold text-gray-900 dark:text-white">
                        {stats.totalArticles > 0 ? Math.round(stats.totalViews / stats.totalArticles) : 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600 dark:text-gray-400">Avg. Shares per Article</span>
                      <span className="text-lg font-bold text-gray-900 dark:text-white">
                        {stats.totalArticles > 0 ? Math.round(stats.totalShares / stats.totalArticles) : 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600 dark:text-gray-400">Articles per Category</span>
                      <span className="text-lg font-bold text-gray-900 dark:text-white">
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                    <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.totalArticles}</p>
                  ...
                  <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Total Shares</p>
                  </div>
                  <div className="text-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
                    <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                      {stats.totalComments.toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Total Comments</p>
                  </div>
                </div>
              </motion.div>
            </>
