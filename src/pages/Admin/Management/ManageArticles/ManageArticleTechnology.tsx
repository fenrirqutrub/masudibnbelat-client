...
const fetch = (page: number, search = "") =>
  axiosPublic
    .get<Resp>("/api/article-technology", {
      params: { page, limit: PAGE_SIZE, search },
    })
    .then((r) => r.data);

const del = (id: string) =>
  axiosPublic.delete(`/api/article-technology/${id}`).then((r) => r.data);

/* ── Component ── */
const ManageArticleTechnology = () => {
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      >
        <div className="flex-1">
          <h2 className="text-2xl font-bold mb-2">Articles Technology</h2>
          <input
            placeholder="Search..."
            value={search}
            onChange={(e) => {
        ...
        <Link to="/add-article-technology">
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.98 }}
            className="px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700"
          >
            + Add
          </motion.button>
        </Link>
...
export default ManageArticleTechnology;
