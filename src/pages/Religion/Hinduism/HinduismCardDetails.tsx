...
// ────────────────────── MAIN COMPONENT ──────────────────────
const HinduismCardDetails = () => {
  const { id } = useParams<{ id: string }>();
  const location = useLocation();
  const stateArticle = location.state?.article as Hinduism | undefined;

  const [article, setArticle] = useState<Hinduism | null>(stateArticle ?? null);
  ...
  useEffect(() => {
    ...
    const fetchArticle = async () => {
      ...
      try {
        setLoading(true);
        const res = await axiosPublic.get<ApiResponse<Hinduism>>(
          `/api/article-hinduism/${id}`
        );
    ...
    const fetchComments = async () => {
      try {
        const res = await axiosPublic.get<ApiResponse<Comment[]>>(
          `/api/article-hinduism/${article._id}/comments`
        );
        if (res.data.success) {
  ...
  const addComment = async () => {
    ...
    try {
      const res = await axiosPublic.post<ApiResponse<Comment>>(
        `/api/article-hinduism/${id}/comments`,
  ...
  const handleShare = async () => {
    ...
    if (!id) return;
    try {
      await axiosPublic.post(`/api/article-hinduism/${id}/share`);
    } catch {
      // optional endpoint
    }
  };
...
export default HinduismCardDetails;
