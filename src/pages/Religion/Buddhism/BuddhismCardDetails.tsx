...
const BuddhismCardDetails = () => {
    ...
    const fetchComments = async () => {
      try {
        const res = await axiosPublic.get<ApiResponse<Comment[]>>(
          `/api/article-buddhism/${article._id}/comments`
        );
        if (res.data.success) {
  ...
  const addComment = async () => {
    ...
    try {
      const res = await axiosPublic.post<ApiResponse<Comment>>(
        `/api/article-buddhism/${id}/comments`,
  ...
  const handleShare = async () => {
    ...
    if (!id) return;
    try {
      await axiosPublic.post(`/api/article-buddhism/${id}/share`);
    } catch {
      // optional endpoint
    }
  };
...
export default BuddhismCardDetails;
