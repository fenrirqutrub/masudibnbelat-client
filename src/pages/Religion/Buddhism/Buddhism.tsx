...
import BuddhismSearch from "./BuddhismSearch";
import BuddhismCard from "./BuddhismCard";
...
const Buddhism = () => {
  ...
  const {
    ...
    queryKey: ["buddhism-articles"],
    queryFn: async () => {
      const res = await axiosPublic.get<ApiResponse>("/api/article-buddhism", {
  ...
  useEffect(() => {
    if (isError) {
      toast.error((error as Error)?.message || "Failed to load articles");
    }
  }, [isError, error]);

  const articles = useMemo(() => response ?? [], [response]);
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 min-h-screen bg-gray-50 dark:bg-[#0D0E14]">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Search Bar */}
        <BuddhismSearch onSearch={setSearchQuery} />
