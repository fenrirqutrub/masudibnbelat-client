...
// ────────────────────── MAIN COMPONENT ──────────────────────
const IslamicCardDetails = () => {
  const { id } = useParams<{ id: string }>();
  const location = useLocation();
  const stateArticle = location.state?.article as Islamic | undefined;

  const [article, setArticle] = useState<Islamic | null>(stateArticle ?? null);
  ...
  useEffect(() => {
    ...
    const fetchArticle = async () => {
      ...
      try {
        setLoading(true);
        const res = await axiosPublic.get<ApiResponse<Islamic>>(
          `/api/article-islamic/${id}`
        );
    ...
    const fetchComments = async () => {
      try {
        const res = await axiosPublic.get<ApiResponse<Comment[]>>(
          `/api/article-islamic/${article._id}/comments`
        );
        if (res.data.success) {
  ...
  const addComment = async () => {
    ...
    try {
      const res = await axiosPublic.post<ApiResponse<Comment>>(
        `/api/article-islamic/${id}/comments`,
  ...
  const handleShare = async () => {
    ...
    if (!id) return;
    try {
      await axiosPublic.post(`/api/article-islamic/${id}/share`);
    } catch {
      // optional endpoint
    }
  };
...
export default IslamicCardDetails;
