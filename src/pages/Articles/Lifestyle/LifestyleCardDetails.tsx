...
const LifestyleCardDetails = () => {
    ...
    const fetchComments = async () => {
      try {
        const res = await axiosPublic.get<ApiResponse<Comment[]>>(
          `/api/articles/lifestyles/${article._id}/comments`
        );
        if (res.data.success) {
  ...
  const addComment = async () => {
    ...
    try {
      const res = await axiosPublic.post<ApiResponse<Comment>>(
        `/api/articles/lifestyle/${id}/comments`,
  ...
  const handleShare = async () => {
    ...
    if (!id) return;
    try {
      await axiosPublic.post(`/api/articls/lifestyle/${id}/share`);
    } catch {
      // optional endpoint
    }
  };
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 rubik-regular">
      <div className="max-w-5xl mx-auto mt-20">
        {/* Back Button */}
        <Link
          to="/articles/lifestyle"
          className="inline-flex items-center gap-2 text-gray-400 dark:text-gray-700 hover:underline mb-6 text-sm font-medium"
        >
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 text-sm text-gray-600 dark:text-[#abc2d3]/80">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium rubik-bold">Masud ibn Belat</span>
            </div>
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <button
              onClick={handleShare}
              className="flex items-center gap-2 px-3 py-2.5 rounded-full text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 transition-all duration-300"
            >
              <Send className="w-4 h-4" />
              <span className="text-sm font-medium">
                {article.views && article.views > 0 ? article.views + 10 : 0}
              </span>
            </button>
          </div>
        </div>

        <div className="border-t border-gray-300 dark:border-slate-900 my-8 w-full"></div>
        ...
        <div className="border-t border-gray-300 dark:border-slate-900 mt-16 mb-8 w-full"></div>
        ...
        <section className="">
          ...
          {/* Add Comment */}
          <div className=" mb-6 flex flex-col md:flex-row justify-between items-center gap-x-5">
            <textarea
              ref={commentInputRef}
              value={commentText}
              onChange={(e) => setCommentText(e.target.value)}
              placeholder="Share your thoughts..."
              className="flex-1 w-full p-3 border border-gray-300 dark:border-slate-900 rounded-lg bg-transparent text-gray-800 dark:text-[#abc2d3] placeholder-gray-500 outline-none resize-none"
              rows={3}
            />
            <button
              onClick={addComment}
              disabled={commenting || !commentText.trim()}
              className=" mt-3 flex items-center gap-2 px-5 py-2.5 rounded-md font-semibold transition-all duration-300 bg-black text-white hover:bg-gray-900 dark:bg-white dark:hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:bg-gray-400 dark:disabled:bg-gray-700 disabled:border disabled:border-black dark:disabled:border-white dark:disabled:text-white dark:text-[#0C0D12] w-full md:w-auto justify-center
  "
            >
              ...
              <div className="space-y-4">
                {comments.map((comment, idx) => (
                  <motion.div
                    ...
                    className="border border-gray-300 dark:border-gray-900 p-5 rounded-lg"
                  >
...
export default LifestyleCardDetails;
