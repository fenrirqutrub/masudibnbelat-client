...
const TravelCardDetails = () => {
    ...
    const fetchComments = async () => {
      try {
        const res = await axiosPublic.get<ApiResponse<Comment[]>>(
          `/api/article-travel/${article._id}/comments`
        );
        if (res.data.success) {
  ...
  const addComment = async () => {
    ...
    try {
      const res = await axiosPublic.post<ApiResponse<Comment>>(
        `/api/article-travel/${id}/comments`,
  ...
  const handleShare = async () => {
    ...
    if (!id) return;
    try {
      await axiosPublic.post(`/api/article-travel/${id}/share`);
    } catch {
      // optional endpoint
    }
  };
